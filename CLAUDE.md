...
```
domain/
  model.rs   — Session, TokenEvent, ToolCall, MetricsSummary 等のエンティティ
  cost.rs    — モデル別単価 (opus/sonnet/haiku) によるコスト計算
  port.rs    — SessionPort / EventPort / OtlpPort トレイト（リポジトリ境界）

infrastructure/
  sqlite/repository.rs    — SqliteRepository（全 Port を実装、std::sync::Mutex<Connection>）
  log_reader/jsonl.rs     — ~/.claude/projects/ JSONL パーサー（usage, tool_use, tool_result 抽出）
  log_reader/scanner.rs   — プロジェクトディレクトリ全走査、lines_processed で差分スキャン
  otlp_reader/parser.rs   — OTLP/JSON スパン・メトリクス型定義とパース
  watcher/notify_watcher.rs — notify クレートによる inotify 監視、2秒デバウンス

application/
  scan_logs.rs    — ScanLogsUseCase（JSONL 走査 → DB 書き込み → サマリー取得）
  ingest_otlp.rs  — IngestOtlpUseCase（OTLP 受信 → DB 書き込み）
  metrics_cache.rs — MetricsCache（tokio::sync::RwLock による /metrics 高速応答キャッシュ）

interface/
  otlp_handler.rs    — axum ルーター（/v1/traces, /v1/metrics, /v1/logs）
  metrics_handler.rs — Prometheus /metrics エンドポイント
  prometheus.rs      — MetricsSummary → Prometheus テキスト形式レンダリング
```
